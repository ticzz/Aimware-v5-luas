...
local function Ondraw()
   ...
   if lp ~= nil and Lve_KeyIndicator_Checkbox:GetValue() then
        ...
        draw.Color( 74, 74, 74, 255)
        if gui.GetValue("rbot.master") and gui.GetValue("rbot.antiaim.base.rotation") >= 0 or gui.GetValue("rbot.antiaim.base.rotation") <= 0 then
			local aarotation = gui.GetValue("rbot.antiaim.base.rotation")
			draw.Color( r2, g2, b2, 255)
		end 
        draw.TextShadow( x+20, y+132, 'AA Rotation  ' .. aarotation .. ' °')
...
local function GetRotation()
	...
	aarotation = gui.GetValue("rbot.antiaim.base.rotation")
	return aarotation

end
callbacks.Register("Draw", GetRotation)
