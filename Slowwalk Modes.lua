...
-- ported from another cheay by https://aimware.net/forum/user/250878
...
callbacks.Register( 'CreateMove', function() 
    ...
    if not bOnGround then
        ffi.cast("CCSGOPlayerAnimationState_534535_t**", ffi_helpers.get_entity_address(entities.GetLocalPlayer():GetIndex()) + 14612)[0].flDurationInAir = 99
        ffi.cast("CCSGOPlayerAnimationState_534535_t**", ffi_helpers.get_entity_address(entities.GetLocalPlayer():GetIndex()) + 14612)[0].flHitGroundCycle = 0
        ffi.cast("CCSGOPlayerAnimationState_534535_t**", ffi_helpers.get_entity_address(entities.GetLocalPlayer():GetIndex()) + 14612)[0].bHitGroundAnimation = false
    end
...
callbacks.Register( 'CreateMove', function()
    ...
    local bOnGround = bit.band(math.floor(m_fFlags), bit.lshift(1,0)) ~= 0
    if bOnGround and not shared_onground then ffi.cast("CCSGOPlayerAnimationState_534535_t**", ffi_helpers.get_entity_address(entities.GetLocalPlayer():GetIndex()) + 14612)[0].flDurationInAir = 0.5 end -- ACT_CSGO_LAND_LIGHT
