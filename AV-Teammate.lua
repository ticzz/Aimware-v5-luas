...
local IG = gui.Groupbox( TAB, "Teammate Invisible", 30 + (640-45)/2, 1565, (640-45)/2, 640 )
local IC = gui.Checkbox( IG, "viewmodel.teaminv", "Enable For Teammate", false )
local ICLR = gui.ColorPicker( IC, "viewmodel.teaminv.clr", "Teammate Color", 255, 255, 255, 255 )
local IT = gui.Combobox( IG, "viewmodel.teaminv.type", "Teammate Material", "Custom","Flat","Chromium","Glow","Crystal" )
local IT_CLR = gui.ColorPicker( IT, "viewmodel.teaminv.type.clr", "Special Teammate Color", 255, 255, 255, 255 )
local IA = gui.Checkbox( IG, "viewmodel.teaminv.additive", "Additive", false )
local ICOLOR = gui.Combobox( IG, "viewmodel.teaminv.colorbase", "Color Modulation Base", "Static","Rainbow","Lerp" )
local ICLR_L_1 = gui.ColorPicker( IG, "viewmodel.teaminv.lerp.clr1", "Lerp From", 255, 255, 255, 255 )
local ICLR_L_2 = gui.ColorPicker( IG, "viewmodel.teaminv.lerp.clr2", "Lerp To", 255, 255, 255, 255 )
local ISPEED = gui.Slider( IG, "viewmodel.teaminv.speed", "Lerp/Rainbow Speed", 10, 1, 100 )
local IRG = gui.Slider( IG, "viewmodel.teaminv.rainbow.gamma", "Rainbow Gamma", 100, 1, 100 )
local IPHONG = gui.Slider( IG, "viewmodel.teaminv.phong", "Phong", 0, 0, 100 )
local IPEARL = gui.Slider( IG, "viewmodel.teaminv.pearl","Pearlescent", 0, 0, 100 )
local IREFLECTIVITY = gui.Slider(IG,"viewmodel.teaminv.reflectivity","Reflectivity",0,0,100)
local IGLOW = gui.Slider( IG, "viewmodel.teaminv.glowint", "Glow Int", 2, 2, 50 )
...
local function IOnDraw()
    ...
    if iold_type ~= type or imat == nil or iold_r ~= r or iold_g ~= g or iold_b ~= b or iold_a ~= a or iold_pearl ~= pearl or iold_phong ~= phong or iold_reflectivity ~= reflectivity or iold_glow ~= glow then
        if type == 0 then
            imat = materials.Create("aw_vm_teaminv",
            [[
                VertexLitGeneric
                {
...
local G = gui.Groupbox( TAB, "Local Teammate Visible", 15, 1565, (640-45)/2, 640 )
local C = gui.Checkbox( G, "viewmodel.team", "Enable For Teammate", false )
local CLR = gui.ColorPicker( C, "viewmodel.team.clr", "Teammate Color", 255, 255, 255, 255 )
local T = gui.Combobox( G, "viewmodel.team.type", "Teammate Material", "Custom","Flat","Chromium","Glow","Crystal" )
local T_CLR = gui.ColorPicker( T, "viewmodel.team.type.clr", "Special Teammate Color", 255, 255, 255, 255 )
local A = gui.Checkbox( G, "viewmodel.team.additive", "Additive", false )
local COLOR = gui.Combobox( G, "viewmodel.team.colorbase", "Color Modulation Base", "Static","Rainbow","Lerp" )
local CLR_L_1 = gui.ColorPicker( G, "viewmodel.team.lerp.clr1", "Lerp From", 255, 255, 255, 255 )
local CLR_L_2 = gui.ColorPicker( G, "viewmodel.team.lerp.clr2", "Lerp To", 255, 255, 255, 255 )
local SPEED = gui.Slider( G, "viewmodel.team.speed", "Lerp/Rainbow Speed", 10, 1, 100 )
local RG = gui.Slider( G, "viewmodel.team.rainbow.gamma", "Rainbow Gamma", 100, 1, 100 )
local PHONG = gui.Slider( G, "viewmodel.team.phong", "Phong", 0, 0, 100 )
local PEARL = gui.Slider( G, "viewmodel.team.pearl","Pearlescent", 0, 0, 100 )
local REFLECTIVITY = gui.Slider(G,"viewmodel.team.reflectivity","Reflectivity",0,0,100)
local GLOW = gui.Slider( G, "viewmodel.team.glowint", "Glow Int", 2, 2, 50 )



local O = gui.Multibox( G, "Overlay" )
local O_WIRE = gui.Checkbox( O, "viewmodel.team.overlay.wire", "Wire", false )
local O_GLOW = gui.Checkbox( O, "viewmodel.team.overlay.glow", "Glow", false)
local O_WIRE_CLR = gui.ColorPicker( G, "viewmodel.team.overlay.wire.clr", "Wire Overlay Color", 255, 255, 255, 255 )
local O_GLOW_CLR = gui.ColorPicker( G, "viewmodel.team.overlay.glow.clr", "Glow Overlay Color", 255, 255, 255, 255 )
local O_GLOW_INT = gui.Slider( G, "viewmodel.team.overlay.glow.int", "Glow Overlay Int", 2, 2, 50 )
...
local function OnDraw()
    ...
    if old_type ~= type or mat == nil or old_r ~= r or old_g ~= g or old_b ~= b or old_a ~= a or old_pearl ~= pearl or old_phong ~= phong or old_reflectivity ~= reflectivity or old_glow ~= glow then
        if type == 0 then
            mat = materials.Create("aw_vm_teammate",
            [[
                VertexLitGeneric
                {
                    $basetexture vgui/white_additive
...
local function Overlay()
    ...
    if o_w == nil then
        o_w = materials.Create("aw_vm_teammate_wire",
        [[
            VertexLitGeneric
            {
                $basetexture vgui/white_additive
