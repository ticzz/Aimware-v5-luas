local LUA_ring = gui.Tab(gui.Reference("Visuals"), "localtab", "Helper")
local configure = gui.Checkbox(LUA_ring, "circle_configure", "Circle Configuration", 0)
local configure_groupbox = gui.Groupbox(LUA_ring, "General")
gui.Text(configure_groupbox, "Sides")
local step_size = gui.Editbox(configure_groupbox, "circle_ring_step", 360)
gui.Text(configure_groupbox, "Range")
local radius = gui.Editbox(configure_groupbox, "circle_ring_radius", 100)
local height = gui.Slider(configure_groupbox, "circle_ring_height", "Height", 30, 0, 100)
local thirdperson_circle = gui.Checkbox(configure_groupbox, "circle_thirdperson_circle", "Show Thirdperson", 1)
local firstperson_circle = gui.Checkbox(configure_groupbox, "circle_firstperson_circle", "Show Firstperson", 1)
local knife_range_active = gui.Checkbox(configure_groupbox, "circle_knife_range_active", "Knife Range", 0)
local zeus_range_active = gui.Checkbox(configure_groupbox, "circle_zeus_range_active", "Taser Range", 0)
local range_smooth = gui.Checkbox(configure_groupbox, "circle_range_smooth", "Smooth", 0)
local range_smooth_delay = gui.Slider(configure_groupbox, "circle_range_smooth_delay", "Smooth Delay", 36, 0, 100)
local configure_groupbox_fps = gui.Groupbox(LUA_ring, "Performance")
local fps_active = gui.Checkbox(configure_groupbox_fps, "circle_fps", "Adaptive Fps", 0)
gui.Text(configure_groupbox_fps, "Target Fps")
local fps_amount = gui.Editbox(configure_groupbox_fps, "circle_fps_amount", 144)
gui.Text(configure_groupbox_fps, "Max Sides")
local fps_max_faces = gui.Editbox(configure_groupbox_fps, "circle_max_faces", 720)
gui.Text(configure_groupbox_fps, "Min Sides")
local fps_min_faces = gui.Editbox(configure_groupbox_fps, "circle_min_faces", 360)
local follow_rotation = gui.Checkbox(configure_groupbox, "circle_follow_rotation", "Follow View Rotation", 0)
local corner = gui.Checkbox(configure_groupbox, "circle_corner", "Corners", 0)
local lines = gui.Checkbox(configure_groupbox, "circle_lines", "Line", 1)
local configure_groupbox_color = gui.Groupbox(LUA_ring, "Color")
local circle_rgb = gui.Checkbox(configure_groupbox_color, "circle_lines", "RGB Fade", 1)
local cricle_collision_active = gui.Checkbox(configure_groupbox, "circle_cricle_collision_active", "Collision", 1)
local circle_red = gui.Slider(configure_groupbox_color, "circle_ring_red", "Red", 255, 0, 255)
local circle_green = gui.Slider(configure_groupbox_color, "circle_ring_green", "Green", 255, 0, 255)
local circle_blue = gui.Slider(configure_groupbox_color, "circle_ring_blue", "Blue", 255, 0, 255)
local circle_alpha = gui.Slider(configure_groupbox_color, "circle_ring_alpha", "Opacity", 255, 0, 255)
local ring_fade_speed = gui.Slider(configure_groupbox_color, "circle_ring_fade_speed", "Fade Speed", 2, 0, 10)
...
callbacks.Register("CreateMove", function(cmd)
    if follow_rotation:GetValue() then
        vec_ViewAngles = EulerAngles( UserCmd.viewangles.x, UserCmd.viewangles.y, UserCmd.viewangles.z)   --Vector3(cmd.viewangles.x,cmd.viewangles.y,cmd.viewangles.z)
    else
        vec_ViewAngles = EulerAngles(0, 0, 0)
    end
end)
...
function drawCircle(Position, Radius)
    ...
    if tonumber(step_size:GetValue()) < 1 then
        gui.SetValue("circle_ring_step", 1)
    end
...
function check_stuff()
    if not configure:GetValue() then
        configure_groupbox:SetInvisible(1)
		configure_groupbox_fps:SetInvisible(1)
		configure_groupbox_color:SetInvisible(1)
	else
        configure_groupbox:SetInvisible(0)
		configure_groupbox_fps:SetInvisible(0)
		configure_groupbox_color:SetInvisible(0)
    end
    ...
    if fps_active:GetValue() then
        ...
        if max_faces ~= nil and fps_cap ~= nil and current_set_faces ~= nil and min_faces ~= nil then
            if fpss < fps_cap and current_set_faces > min_faces then
                gui.SetValue("circle_ring_step", current_set_faces - 1)
...
callbacks.Register("Draw", "check_stuff", check_stuff)
...
--***********************************************--
print("♥♥♥ " .. GetScriptName() ..  " without Errors ♥♥♥" );
