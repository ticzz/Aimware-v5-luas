...
local pos = gui.Reference("Ragebot", "Anti-Aim", "Advanced");
...
return 
    ...
    if pLocal and pLocal:IsAlive() and enabled:GetValue()  then
        ...
        if lastChoked and choking then iHateMyself(lastChoked.y, {25, 255, 25, 255}, "Last Choked") end
        if fake then iHateMyself(fake.y, {255, 25, 25, 255}, "Networked") end
        if localAngle then iHateMyself(localAngle.y, {25, 25, 255, 255}, "Local Angle") end
        if lby then iHateMyself(lby, {255, 255, 255, 255}, "LBY") end
    end
end)
